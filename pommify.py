...
# Many thanks to https://github.com/hyperreality/ for the dict.
aebe = {
...
def ae(text, to_british):
    compiled_patterns = []
    for british, american in aebe.items():
        if to_british:
            compiled_patterns.append((re.compile(rf'(?<![\\{{])\b{american}\b'), british))
            compiled_patterns.append((re.compile(rf'(?<![\\{{])\b{american.capitalize()}\b'), british.capitalize()))
        else:
            compiled_patterns.append((re.compile(rf'(?<![\\{{])\b{british}\b'), american))
            compiled_patterns.append((re.compile(rf'(?<![\\{{])\b{british.capitalize()}\b'), american.capitalize()))
    
    for pattern, replacement in compiled_patterns:
        text = pattern.sub(replacement, text)
    return text

def pfy(fpath, to_british):
    with open(fpath, 'r') as file:
        text = file.read()

        if re.search(r'\\begin{document}', text) and re.search(r'\\end{document}', text):
            preamble = text[:re.search(r'\\begin{document}', text).end()]
            txt = text[re.search(r'\\begin{document}', text).end():re.search(r'\\end{document}', text).start()]
            postscript = text[re.search(r'\\end{document}', text).start():]

        elif re.search(r'\\begin{document}', text):
            preamble = text[:re.search(r'\\begin{document}', text).end()]
            txt = text[re.search(r'\\begin{document}', text).end():]
            postscript = ''

        elif re.search(r'\\end{document}', text):
            preamble = ''
            txt = text[:re.search(r'\\end{document}', text).start()]
            postscript = text[re.search(r'\\end{document}', text).start():]
        ...
        return preamble + ae(txt, to_british) + postscript
    
def pommify(fpath, to_british = True):
    with open(fpath.replace('.tex', '_pommified.tex'), 'w') as file:
        file.write(pfy(fpath, to_british))